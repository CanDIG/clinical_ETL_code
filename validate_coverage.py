...
def validate_coverage(map_json, input_path=None, verbose=False):
    ...
    if "openapi_url" not in map_json:
        print("No openapi_url schema available")
        return None
    schema = MoHSchema(map_json["openapi_url"])
    if schema is None:
        print(f"Did not find an openapi schema at {map_json['openapi_url']}; please check link")
        return None
...
def main(args):
    if args.json is not None and os.path.isfile(args.json):
        ...
        print("A JSON file, generated by CSVConvert.py, is required, using the --json argument")
        return
